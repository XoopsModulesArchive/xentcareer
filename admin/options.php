...
function reference_job()
{
    ...
    $fct1 = 'xent_cr_reference';   	//table ds la bd
...
";
    ...
    $result = $xoopsDB->query('SELECT id, reference_job FROM ' . $xoopsDB->prefix('xent_cr_reference') . ' ORDER BY id');
...
<input type='hidden' name='op' value='ADDOPTIONS'><input type='submit' value='" . _AM_XENT_CR_ADD . "'>";
    ...
    $result = $xoopsDB->query('SELECT id_status, status FROM ' . $xoopsDB->prefix('xent_cr_status') . ' ORDER BY id_status');
