...
if (empty($_POST['submit'])) {
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'xentcareer_refform.html';
    ...
    $postuler = new XoopsThemeForm(_MI_XENT_CR_FORMAPPLY, 'postuler', 'refform.php');
    ...
    $postuler->addElement(new xoopsFormLabel('', _MI_XENT_CR_REFRECOMMANDFULL));
    ...
    $postuler->addElement(new xoopsFormLabel('<br><hr>', '<br><hr><br>'));
    ...
    if (!empty($field) || '' != $field) {
        if (('' == $_FILES[$field]['tmp_name'] || !is_readable($_FILES[$field]['tmp_name']))) {
            redirect_header("refform.php?job_id=$job_id&prenom=$prenom&nom=$nom&email=$email&address=$address&ville=$ville&zipcode=$zipcode&telcell=$telcell&telhome=$telhome&telautre=$telautre&heardodesia=$heardodesia&nomress=$nomress&emailress=$emailress", 4, sprintf(_MI_XENT_CR_SENDDOCTYPEERROR, $xoopsModuleConfig['cv_extention'], $xoopsModuleConfig['maxfilesize']) . '<br>CV');

            //redirect_header( "refform.php?job_id=$job_id" , 4, _MI_XENT_CR_SENDDOCTYPEERROR." CV" ) ;
    ...
    /*if ($HTTP_POST_FILES['rec']['name'] != "") {
        if( !empty( $field1 ) || $field1 != "" ) {
            ...
            if( $_FILES[$field1]['tmp_name'] == "" || ! is_readable( $_FILES[$field1]['tmp_name'] ) ) {
                ...
                redirect_header( "refform.php?job_id=$job_id&prenom=$prenom&nom=$nom&email=$email&address=$address&ville=$ville&zipcode=$zipcode&telcell=$telcell&telhome=$telhome&telautre=$telautre&heardodesia=$heardodesia&nomress=$nomress&emailress=$emailress" , 4, sprintf(_MI_XENT_CR_SENDDOCTYPEERROR, $xoopsModuleConfig['cv_extention'], $xoopsModuleConfig['maxfilesize'])."<br>LR" ) ;
                //redirect_header( "refform.php?job_id=$job_id" , 4, _MI_XENT_CR_SENDDOCTYPEERROR." LR" ) ;
                exit ;
            }
